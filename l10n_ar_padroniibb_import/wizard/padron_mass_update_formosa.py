...
class PadronMassUpdateFormosa(models.TransientModel):
    ...
    # Percepcion formosa
    @api.model
    def _update_perception_formosa(self, perception):
        cr = self.env.cr
        query = """
        WITH padron AS (
            SELECT
                rp.id p_partner_id,
                par.ac_per_23_14 p_percentage
            FROM res_partner rp
    ...
    def action_update_formosa(self):
        ...
        if self.formosa:
            ...
            percep_formosa = perception_obj._get_perception_from_formosa()
            if not percep_formosa:
                raise ValidationError(
                    _("Perception Error!\n") +
                    _("There is no perception configured to update " +
                      "from Padron FORMOSA"))
            self._update_perception_formosa(percep_formosa[0])
